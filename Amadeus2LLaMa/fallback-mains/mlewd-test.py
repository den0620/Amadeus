...
LLM_USERS=[573799615074271253,527505544261664798,396961790778540032,579639623668727808,489895341496467456,547738827410898965,676476841707700235,677151784795504651]
    ...
    if message.channel.id in LLM_CHANNELS:
        if message.author.id in LLM_USERS:
            ...
            if LLM_LOCK==0:
                ...
                llmContext=f"Below is a fraction of discord text channel \"{message.channel.name}\" at \"{message.guild.name}\" discord server (guild). \"{clientUser.display_name}\" is horny assistant (she) and running \"{llmModel}\" llm via llama-cpp. Her current memory is {discordLimit} messages.\n\n"
