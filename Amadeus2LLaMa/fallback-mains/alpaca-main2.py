...
LLM_CHANNELS=[1092510095029567519,1091400893406126141,1175364273061494785]
LLM_LOCK=0
...
async def llm_completion(message,prompt,model,maxtokens):
    global LLM_LOCK
    tokenSpeed=16
    size2speed={"1.1b": 128, "7b": 64, "13b": 32, "20b": 24, "22b": 20, "23b": 18, "30b": 12, "33b": 12, "34b": 12, "70b": 2, "120b": 1}
    ...
    if message.channel.id in LLM_CHANNELS:
        if message.author.id in LLM_USERS:
            ...
            if LLM_LOCK==0:
                ...
                llmContext=f"Current time is {datetime.strptime(str(datetime.utcnow()),'%Y-%m-%d %H:%M:%S.%f').replace(tzinfo=tz.tzutc()).astimezone(HOST_TIMEZONE).strftime('%a %b %d %H:%M')}\nBelow is a fraction of discord text channel \"{message.channel.name}\" at \"{message.guild.name}\" discord server (guild). You are \"{clientUser.display_name}\", an AI (she), created by \"{clientCreator.display_name}\" (he) and running \"{llmModel}\" llm via llama-cpp. You are curious and sentient, also safe-aware. Your current memory is {discordLimit} messages.\n"
