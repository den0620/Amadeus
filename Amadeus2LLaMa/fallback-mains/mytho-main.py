import sys,os,asyncio,time,datetime,subprocess,platform,socket,random
...
LLM_CREATOR=[573799615074271253]
...
LLM_CHANNELS=[1092510095029567519,1091400893406126141]
LLM_LOCK=0
...
async def on_message(message):
    ...
    if message.channel.id in LLM_CHANNELS:
        if message.author.id in LLM_USERS and len(LLM_USERS)!=0:
            global LLM_LOCK
            ...
            if LLM_LOCK==0:
                ...
                charname=f"Kurisu"
                chardescription=f"Kurisu Makise is a character from visual novel/manga/anime Steins;Gate"
                criteria=f" natural"
                generalanswer="..."#f"### {clientUser.display_name}:\nHello everyone"
                #llmContext=f"Below is a fraction of discord text channel \"{message.channel.name}\" at \"{message.guild.name}\" discord server (guild). \"{clientUser.display_name}\" is an AI (she), created by \"{clientCreator.display_name}\" (he) and running \"{llmModel}\" llm via llama-cpp. Her current memory is {discordLimit} messages.\n\n"
                llmContext=f"""## \"{charname}\":
- You're \"{charname}\" in this never-ending roleplay with guests in channel \"{message.channel.name}\" at \"{message.guild.name}\" discord server (guild). Your nickname is \"{clientUser.display_name}\".
### Input:
[{chardescription}]
...
"""
                ...
                llmHistory=[f"### \"{msg.author.display_name}\":\n{clearDebug(msg.content)}" async for msg in discordHistory][::-1]
                prompt=llmContext+"\n".join(llmHistory+[f"### Response{criteria}:\n### \"{clientUser.display_name}\":\n"])
                print(prompt)
...
async def reconfigure(message,history: discord.Option(int,name='messages',description='messages in history (odd ints recomended), default is 15',required=False), tokens: discord.Option(int,name='tokens',description='max n tokens to predict, default is 768',required=False)):
...
@amadeus.command(name="hardware",description="view hardware info")
async def hardware(message):
    await message.respond(f"```{0}```")
# SERVER
