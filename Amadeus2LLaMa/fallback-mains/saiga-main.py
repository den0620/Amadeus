...
async def llm_completion(message,prompt,model,maxtokens):
    global LLM_LOCK
    tokenSpeed=16
    size2speed={"1.1b": 128, "7b": 64, "13b": 32, "30b": 12, "33b": 12, "34b": 12, "70b": 2}
    ...
    if message.channel.id in LLM_CHANNELS:
        if message.author.id in LLM_USERS:
            ...
            if LLM_LOCK==0:
                ...
                llmContext=f"Ниже - отрывок из чата \"{message.channel.name}\" на дискорд сервере \"{message.guild.name}\". \"{clientUser.display_name}\" - ИИ (она), созданная \"{clientCreator.display_name}\" (он), и работающая на \"{llmModel}\" LLM через llama-cpp. Её память {discordLimit} сообщений.\n\n"
